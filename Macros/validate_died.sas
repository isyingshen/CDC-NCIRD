/*************************************************************************************************/
/* Description: This macro extracts Stage2 and Stage3 Died variables from                        */
...
%macro validate_died;
   %if (&eventcode =10150 or &eventcode =10590 or &eventcode =11723 or &eventcode =11720 or 
        &eventcode =11717) %then 
   %do;
      proc sql noprint;
         select stg2netss, stg3netss
         into :stg2netss1-, :stg3netss1-
         from var_list
         where format_type = 'DIED'
         ;
         %let total = &sqlobs;
      quit;  
      ...
      data mapping_validation;
         ...
         %do i=1 %to &total;	
            ...
            else if (&&stg2netss&i = '1' and &&stg3netss&i = 'N') then
               results_&&stg2netss&i.. = 'OK';
            else if (&&stg2netss&i = '2' and &&stg3netss&i = 'Y') then
               results_&&stg2netss&i.. = 'OK';
